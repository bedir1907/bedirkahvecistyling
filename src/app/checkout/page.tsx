...
export default function CheckoutPage() {
  ...
  async function handleSubmit(e: React.FormEvent<HTMLFormElement>) {
    ...
    if (!form.name.trim() || !form.email.trim() || !form.phone.trim() ||
      !form.city.trim() || !form.district.trim() || !form.address.trim()) {
      setError("Lütfen teslimat için zorunlu alanları doldur")
      return
    }

    if (!billingSameAsShipping) {
      if (!billingForm.billingName.trim() || !billingForm.billingPhone.trim() ||
        !billingForm.billingCity.trim() || !billingForm.billingDistrict.trim() ||
      ...
      <section className="max-w-7xl mx-auto px-4 py-10">
        <div className="grid lg:grid-cols-[1.15fr_420px] gap-8 items-start">
          ...
          >
            ...
            {loadingCustomer ? (
              <div className="text-sm text-gray-500">Bilgiler hazırlanıyor...</div>
            ) : null}
            ...
            <p className="text-xs text-gray-500 leading-6">
              ...
              <Link href="/mesafeli-satis-sozlesmesi" className="underline hover:text-black transition">
                Mesafeli Satış Sözleşmesi
              </Link>{" "}
              ve{" "}
              <Link href="/mesafeli-satis-on-bilgilendirme" className="underline hover:text-black transition">
          ...
          <aside className="bg-white border border-black/10 rounded-[28px] p-6 md:p-8 lg:sticky lg:top-24">
            ...
            {/* Kargo + Toplam */}
            <div className="pt-4 mt-4 border-t border-black/10 space-y-2.5">
              ...
              {shipping.freeAbove && shippingCost > 0 && (
                <p className="text-xs text-green-600">
                  {formatPrice(shipping.freeAbove)} ve üzeri siparişlerde ücretsiz
                </p>
              )}
